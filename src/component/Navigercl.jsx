...
const Naviger = ({ nom }) => {
  ...
  const handleFormSubmit = async (event) => {
    ...
    const eventData = {
       // Store the event name entered by the user
      eventDate: selectedDate, // Directly use the Date object
    };
      try {
          const response = await axios.post('http://localhost:7000/resclient/add', formData);
          console.log('Server response:', response.data);
          const successMessage = document.createElement('p');
            successMessage.textContent = 'Reservation ajoutée';
            ...
            form.appendChild(successMessage);
        {
            ...
            console.error('Failed to add reservation:', response.data.error);
        }
  ...
  function AvisForm() {
    ...
    const foundService = services.find(service => service._id === formData.serviceId);
    console.log("Selected Service ID:", formData.serviceId);

const filteredCategories = foundService ? foundService.categories : [];
      ...
      <div className="row">
        ...
        {service.map((services, index) => (
              <div key={index} className="col-lg-3 col-md-6 mt-5 mt-md-0">
                <FlipCard
                  frontContent={<div><h4>{services.name}</h4></div>}
                  backContent={
                    <div>
                      <h4>Categories</h4>
                      <ul>
 ...
 <section id="appointment" className="appointment section-bg">
  ...
  <section id="appointment" className="appointment section-bg">
    <div className="container">
      ...
      <Avisform setSuccessMessage={setSuccessMessage} />
      {successMessage && (
        <p>{successMessage}</p>
      )}
    </div>
